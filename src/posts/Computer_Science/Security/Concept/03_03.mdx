...
3. 항등원: $\exists e \in G, \forall a \in G, a \circ e = e \circ a = a$
4. 역원: $\forall a \in G, \exists a^{-1} \in G, a \circ a^{-1} = a^{-1} \circ a = e$
5. 교환 법칙(abelian): $\forall a, b \in G, a \circ b = b \circ a$
...
- $(\mathbb{Z}_n^*, \times)$: 양의 정수 $n$에 대해 아벨 군 ($|\mathbb{Z}_n^*| = \phi(n)$, 여기서 $\phi$는 [오일러 파이](#오일러-파이-함수-eulers-phi-function) 함수)

## 원소 차수 (Order of Element)
> $(\mathbb{G}, \circ)$가 군이고 $a \in \mathbb{G}$일 때, $a$의 차수는 $a$를 반복해서 연산하여 항등원 $e$를 만들 때 필요한 최소 횟수이다.
...
- $3^4 = 15 = 4 \;(\text{mod}\; 11)$
- $3^5 = 12 = 1 \;(\text{mod}\; 11)$
- 따라서 $\text{ord}(3) = 5 \;in\; (\mathbb{Z}_{11}^*, \times)$

## 순환 군 (Cyclic Group)
> 최대 차수 $\operatorname{ord}(g) = |\mathbb{G}|$를 가진 원소 $g$를 포함하는 군 $\mathbb{G}$를 순환 군이라고 한다. $g$를 발생자 또는 원시 원소라고 부른다.
...
이산 로그 문제는 이러한 연산을 근사적으로 계산하는 것이 어려움
- $\mathbb{Z}_{31}^*$에서 $g = 3$, $A = 20$일 때, $3^x\mod{31} = 20$을 만족하는 $x$를 찾는 것은 어려움
