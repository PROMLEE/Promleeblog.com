...
3. $\phi(N) = (p-1) * (q-1)$를 계산합니다. $\phi(N)$은 N과 서로소인 수의 개수를 의미함
	[오일러 파이 함수](/blog/post/28-essential-num-theory#오일러-파이-함수-eulers-phi-function)
...
따라서 $M' = C^d = 22^{77} = (22^{11})^{7} = 61^{11} = 3$ (mod 221)
...
[중국인의 나머지 정리](/blog/post/28-essential-num-theory#중국인의-나머지-정리-chinese-remainder-theorem)를 사용하여
$M_p = 17^{-1} = 10 \pmod{13}$
$M_q = 13^{-1} = 4 \pmod{17}$
